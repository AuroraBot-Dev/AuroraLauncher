...
- State lives in a portable `tool/` folder next to the executable — `<exe dir>\tool` on Windows — with the sub-tree built in `state.rs::RuntimePaths` (state, downloads, staging, tools, env, kernel, home, cache, logs). Deleting that `tool` dir fully resets the launcher; it never touches host PATH/registry.
