<template>
  <n-drawer :show="show" :width="380" placement="right" :auto-focus="false" @update:show="emit('update:show', $event)">
    <n-drawer-content title="关于" :native-scrollbar="false">
      <n-flex vertical :size="18">
        <n-flex vertical align="center" :size="8">
          <n-text strong style="font-size: 18px">AuroraLauncher</n-text>
        ...
        <n-flex :size="8">
          ...
          <n-button style="flex: 1" @click="copyDiagnostics">
            复制诊断信息
          </n-button>
        </n-flex>
          ...
          <n-collapse-item title="法律与许可" name="legal">
            <n-flex vertical :size="10">
              ...
              <n-text depth="3" style="font-size: 12px; line-height: 1.7">
                隐私说明：不收集任何个人信息，运行数据均保存在本地 tool 目录；仅在检查更新时访问 GitHub Releases。
              </n-text>
            </n-flex>
          </n-collapse-item>
        </n-collapse>

        <n-text depth="3" style="font-size: 12px; text-align: center">© 2026 AuroraBot</n-text>
      </n-flex>
    </n-drawer-content>
  </n-drawer>
</template>
